...
- **[2026 年 8 月 14 日]** **[v0.2.7](https://github.com/EvoScientist/EvoScientist/releases/tag/v0.2.7)** — 启动更快：重型依赖延迟加载、langgraph dev 服务可保活跨会话复用（`langgraph_dev_keepalive`）、会话列表改用索引查询；配置了加密 webhook 的渠道现在拒绝未签名的 POST；MCP stdio 服务在重定向流下可正常启动（修复 Windows Errno 9）；子 agent 工具改在派发时解析，避免配置加载期误报与丢失；`/compact` 历史 offload 适配 deepagents 0.7.6，按会话独立保存历史文件。
