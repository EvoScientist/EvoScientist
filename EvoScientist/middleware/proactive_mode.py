...
``langgraph.config.get_config()``), not a server-side thread-state store — a
per-run signal belongs on the run's config, not on state that outlives it.
Template: ``middleware/active_team.py``, which reads ``configurable.active_teams``
the same way.
