...
def _cache_key(provider: str, base_url: str | None) -> str | None:
    ...
    Returns:
        String key like ``"openai|https://api.openai.com/v1"``, or ``None``
        if the base URL cannot be resolved.
    """
...
def _get_cache_entry(provider: str, base_url: str | None = None) -> dict | None:
    """Retrieve the raw cache dictionary for *provider* and *base_url*."""
...
def _save_cache_entry(provider: str, base_url: str | None, models: list[str]) -> None:
    """Write a list of model IDs to the local cache for *provider*."""
    key = _cache_key(provider, base_url)
    if not key:
        return
    cache = _load_cache()
...
def _resolve(
    ...
    *,
    ...
    Handles environment variable lookups and default base URLs for all
    supported providers.
    """
    ...
    if base_url:
        ...
        resolved_base_url = (
            os.environ.get("CUSTOM_ANTHROPIC_BASE_URL", "").rstrip("/") or None
        )
    else:
...
def fetch_models(
    ...
    *,
    ...
    .. warning::
        This performs a synchronous, blocking HTTP request when the cache is
        missed. In coroutine contexts (like Discord/Telegram channels), use
        :func:`fetch_models_async` instead to avoid blocking the event loop.
