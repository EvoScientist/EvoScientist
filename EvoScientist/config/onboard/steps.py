...
def _step_webui(config: EvoScientistConfig) -> dict[str, object]:
    """Configure the deploy-mode WebUI control API."""
    console.print("\n  [bold cyan]── WebUI Deploy API ──[/bold cyan]")
    ...
    bind_host = questionary.text(
        "WebUI bind host (127.0.0.1 = local only):",
    ...
    langgraph_port = int(getattr(config, "langgraph_dev_port", 6174) or 6174)
    while True:
        ...
        if not 1 <= port <= 65535:
            ...
            continue
        if port == langgraph_port:
            console.print(
                f"  [red]Port {port} is already used by LangGraph Dev. "
                "Pick another port.[/red]"
            )
            continue
    ...
    allowed_origins = questionary.text(
        "Allowed WebUI frontend origins (comma-separated, empty = localhost dev):",
    ...
    allowed_hosts = questionary.text(
        "Allowed WebUI backend hosts "
        "(comma-separated, empty = direct local/private IPs):",
