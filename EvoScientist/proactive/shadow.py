...
- **Isolation** — the shadow uses an ``InMemorySaver`` checkpointer, so its
  ...
  middleware chain established that the tool surface plus these three
  writer-gating flags is the whole side-effect axis: every other writer is
  either tool-backed and thus killed by the strip, or gated by one of them).
  ``_shadow_cfg`` additionally sets ``auto_mode`` / ``auto_approve`` /
  ``model_fallbacks`` — not side-effect gates but the unattended-run contract
  (see ``_SHADOW_FLAGS``).
...
# The side-effect axis: the first three flags gate a writer identified by the
# middleware audit; ``auto_mode``/``auto_approve`` declare the unattended-run
# contract and ``model_fallbacks`` pins the shadow to its primary model.
# The reasoning axis (prompt, memory injection, model, context) is left
...
def build_shadow_graph(
    ...
    non-None → none of the cached config/model globals are written; it does still
    set the active workspace and seed the fallback chain, which is why
    ``workspace_dir`` must be the live one). ``workspace_dir`` MUST be the
...
_CLEAN_FINISH_REASONS = frozenset({"stop", "end_turn", "STOP", "stop_sequence"})
...
def _finish_reason(message: Any | None) -> str | None:
    # Providers disagree on the key: OpenAI writes ``finish_reason``, Anthropic
    # (the default provider) writes ``stop_reason`` — read both, or an Anthropic
    # ``max_tokens`` truncation reads as clean and ships as a push.
