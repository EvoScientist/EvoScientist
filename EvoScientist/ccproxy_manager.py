...
def write_ccproxy_config() -> str:
    """Write the ccproxy config file EvoScientist passes via ``CONFIG_FILE``.

    Two Codex plugin defaults are turned off:
...
def start_ccproxy(port: int) -> subprocess.Popen:
    ...
    # ccproxy's app factory re-reads settings from CONFIG_FILE and ignores --config.
    env = os.environ.copy()
    try:
        env["CONFIG_FILE"] = write_ccproxy_config()
