...
class TestStepChannels:
    ...
    def test_webui_deploy_rejects_occupied_port(self):
        """Test WebUI setup asks again when the selected port is not bindable."""
        from EvoScientist.config.onboard.steps import _step_webui
        ...
        with (
            ...
            result = _step_webui(config)

        assert result["webui_enabled"] is True
    ...
    def test_webui_deploy_rejects_langgraph_port(self):
        """WebUI must not bind to the same port as LangGraph Dev."""
        from EvoScientist.config.onboard.steps import _step_webui

        config = EvoScientistConfig(langgraph_dev_port=6174)
        ...
        assert result["webui_port"] == 8011
        mock_can_bind.assert_called_once_with(8011)

    def test_webui_deploy_requires_api_key_for_non_loopback_bind(self):
        """Public WebUI binds require an API key before onboarding accepts them."""
        from EvoScientist.config.onboard.steps import _step_webui
        ...
        with (
            ...
            mock_q.password.return_value.ask.side_effect = ["", "secret"]
            result = _step_webui(config)

        assert result["webui_api_key"] == "secret"
    ...
    def test_returns_true_on_save(self):
        ...
        with (
            ...
            mock_q.confirm.return_value.ask.side_effect = [
                False,  # WebUI deploy API: disabled
                True,  # Save config
            ]
            mock_q.text.return_value.ask.side_effect = [
