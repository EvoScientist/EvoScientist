...
def test_create_tool_selector_no_tracker_when_not_tracking():
    """When track_stream_selection=False, only the conditional middleware is returned."""
    p1, p2, p3 = _factory_patches()
    with p1, p2, p3:
...
def test_create_tool_selector_tracker_when_tracking():
    """When track_stream_selection=True (default), both middlewares are returned."""
    p1, p2, p3 = _factory_patches()
    with p1, p2, p3:
        result = create_tool_selector_middleware(track_stream_selection=True)
    assert isinstance(result, list)
    assert len(result) == 2
...
def test_conditional_middleware_track_flag_propagated_when_false():
    """The track_stream_selection=False is forwarded to the conditional middleware."""
    p1, p2, p3 = _factory_patches()
    with p1, p2, p3:
        result = create_tool_selector_middleware(track_stream_selection=False)
    assert result[0]._track_stream_selection is False


def test_conditional_middleware_track_flag_propagated_when_true():
    """The track_stream_selection=True is forwarded to the conditional middleware."""
    p1, p2, p3 = _factory_patches()
    with p1, p2, p3:
        result = create_tool_selector_middleware(track_stream_selection=True)
    assert result[0]._track_stream_selection is True
