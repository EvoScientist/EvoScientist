...
def test_finished_cleanly_reads_anthropic_stop_reason():
    """Anthropic reports the terminal reason under ``stop_reason`` (the default
    provider); a ``max_tokens`` truncation must be judged non-clean (#263 review)."""
    from langchain_core.messages import AIMessage

    from EvoScientist.proactive.shadow import _finished_cleanly

    assert _finished_cleanly(
        AIMessage(content="x", response_metadata={"stop_reason": "end_turn"})
    )
    assert not _finished_cleanly(
        AIMessage(content="x", response_metadata={"stop_reason": "max_tokens"})
    )
    # the OpenAI key still works
    assert not _finished_cleanly(
        AIMessage(content="x", response_metadata={"finish_reason": "length"})
    )
