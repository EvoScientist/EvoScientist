...
def test_default_middleware_includes_proactive_mode_for_main_agent(
    ...
    mock_config.return_value = EvoScientistConfig(proactive_enabled=True)
...
@patch("EvoScientist.EvoScientist._ensure_chat_model")
@patch("EvoScientist.EvoScientist._ensure_config")
def test_default_middleware_excludes_proactive_mode_when_disabled(
    mock_config, mock_model
):
    """Off by default: the layer isn't added, so a defaults-off stack pays no
    per-call get_config()."""
    from EvoScientist.config.settings import EvoScientistConfig

    mock_config.return_value = EvoScientistConfig(proactive_enabled=False)
    mock_model.return_value = MagicMock(profile={"max_input_tokens": 200_000})
...
def test_proactive_mode_is_outer_of_tool_selector(mock_config, mock_model):
    ...
    selector_index = next(
        ...
        if isinstance(m, _ConditionalToolSelectorMiddleware)
    )
