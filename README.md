...
- **[14 Aug 2026]** **[v0.2.7](https://github.com/EvoScientist/EvoScientist/releases/tag/v0.2.7)** — Faster startup: heavy imports load lazily, the langgraph dev server can stay alive and be reused across sessions (`langgraph_dev_keepalive`), and thread listing uses an indexed query; channels configured for encrypted webhooks now reject unsigned POSTs; MCP stdio servers start reliably under redirected streams (fixes Windows Errno 9); sub-agent tools resolve at dispatch time instead of config load; `/compact` history offload restored on deepagents 0.7.6, which keeps a separate history file per session.
